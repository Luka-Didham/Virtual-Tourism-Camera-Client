...
namespace Byn.Unity.Examples
{
    ...
    /// 1. The system starts and shows a Receiver & Sender button. Press the sender button first.
    /// 2. Camera is accessed locally (depending on platforms it asks for permissions)
    /// 3. The local camera should be shown now.
    /// 4. Press the receiver button locally or on a different device to receive the stream
    /// 5. You can press the receiver button several time to test the performance with several streams.
    /// 
    ...
    {
        /// <summary>
        /// The receivers wait x sec until trying to connect to the sender to give the sender a headstart
        /// </summary>
        private readonly int RECEIVER_CONNECT_DELAY = 2;
        ...
        private IEnumerator InitExample()
        {
            if (sAddress == null)
            {
                sAddress = Application.productName + "_" + Application.companyName;
            }
            ...
            else
            {
                ...
                //wait a while before trying to connect other sender
